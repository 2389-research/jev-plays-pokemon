...
- **Stuck detector — freeze, don't reset:** pass `forced_movement=True` (its existing suppression path) when `in_conversation`; in `_finish`, a conversation-suppressed step leaves `_blocked_for_n` **unchanged** (neither increments nor resets). Resetting would make a dialogue loop (re-talking a blocking NPC, bumping a sign) unescapable, because every cycle would zero the block budget; freezing keeps it accumulating across the overworld steps of the cycle. Accepted latency: only one overworld step per dialogue cycle feeds the position-oscillation check, so a same-NPC dialogue loop reaches `BLOCK_TRIGGER` after ~12 cycles (bounded at ≤15 in its test). The existing battle/forced-movement suppression semantics are left as-is. The budget update is factored into a small method so it is unit-testable.
