...
- **Timeout (never stall forever):** `self._forced_waits` counts **consecutive F5-forced Waits** only; any other step (a dialogue advance, a normal nav step) resets it to 0. If it reaches `FORCED_WAIT_MAX_STEPS = 40`, emit `script_wait_timeout`, fall through to normal handling, and **disarm** F5-forced waiting until `wJoyIgnore` is 0 and no dialogue has been seen for the grace window (then re-arm). This covers every cause of a forced wait (a stuck `wJoyIgnore` *or* a false-positive dialog read). Evidence it won't fire in normal play: across 1200 anchors of the 1839 run `wJoyIgnore` was non-zero only briefly (the scripted auto-step leaving the lab); in the Pokédex cutscene it stays `0xFC` for 27+ steps but those are mostly dialogue steps, which reset the counter.
- **Stuck detector — freeze, don't reset:** pass `forced_movement=True` (its existing suppression path) when `in_conversation`; in `_finish`, a conversation-suppressed step leaves `_blocked_for_n` **unchanged** (neither increments nor resets). Resetting would make a dialogue loop (re-talking a blocking NPC, bumping a sign) unescapable, because every cycle would zero the block budget; freezing keeps it accumulating across the overworld steps of the cycle. The existing battle/forced-movement suppression semantics are left as-is. The budget update is factored into a small method so it is unit-testable.
...
- `states/pallet_route1_edge.state` ← `runs/brock-run-20260922-1839/states/map0_step15.state` (Pallet, one step below the Route 1 edge)
