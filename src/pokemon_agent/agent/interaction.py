...
def distances(start: Tile, *, walkable, blocked=(), cuts=(), terrain=None, ledge_ok=None, spins=None) -> dict[Tile, int]:
...
def plan(player: Tile, target: Tile, *, walkable, occupied: dict[Tile, str], warps=(), counters=(),
         cuts=(), terrain=None, face: str | None = None, ledge_ok=None, spins=None) -> list[Side]:
    ...
    dist = distances(tuple(player), walkable=walkable, blocked=set(others) | (set(warps) - {tuple(player)}),
                     cuts=cuts, terrain=terrain, ledge_ok=ledge_ok, spins=spins)
    out = []
