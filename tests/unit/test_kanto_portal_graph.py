...
def test_no_story_gates_in_data_and_observed_blocks_reroute():
    """runs/ss-anne-20260923: a hand-written gate ("officer blocks the back door early on") was wrong and
    silently vetoed the only route south of Cerulean. The data carries no story gates; a portal the
    agent OBSERVED blocked (PortalGraph.blocked, set at runtime) is routed around instead."""
    from pokemon_agent.agent.portal_graph import PortalGraph
    g = PortalGraph.load()
    assert not any(p.get("gated") or p.get("open_when") for p in g.portals.values())
    comp = g._grid(3)[(19, 18)]                              # main Cerulean, outside the Pokécenter
    r = g.route(3, comp, 5)
    assert r and r[0]["dest_map"] == 62                      # through the trashed house
    g.blocked = {"ceruleancity:warp1": "a Guard at (27,12) blocks it"}
    assert g.route(3, comp, 5) is None                       # the only way is blocked -> honest None
